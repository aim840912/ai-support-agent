...
10. **There is only one database — local development writes to production.** `DATABASE_URL` in `.env.local` and the one on Vercel point at the same Neon instance. Nothing in the repo reveals this. Three consequences: seeded test data appears on the live site; a test user created locally is a real login on production (delete it before deploying); and `INTEGRATION_ENCRYPTION_KEY` **must be identical** in both places, or a Telegram bot token encrypted in one environment fails to decrypt in the other. Audit with a throwaway Prisma script before any deploy.
11. **Vercel environment variables lag the code.** `OPENROUTER_API_KEY` was absent for some time after the project moved off Groq, so the live site silently ran in mock mode; `NEXT_PUBLIC_APP_URL` is documented as required here but had never been set, which made webhook payloads carry `http://localhost:3000` links. Changing a variable does **not** affect finished deployments — redeploy after editing one. Check with `vercel env ls production`.
12. **A new server-to-server API route answers 307 → `/login` instead of running.** Symptom is a redirect plus two `authjs.*` cookies where you expected JSON — it looks nothing like an auth failure, and no error is logged. Cause: the NextAuth proxy guards everything not listed in `publicPrefixes` in `src/auth.config.ts`. Any endpoint that carries its own credential (webhook secret, API key, bearer token) instead of a session cookie **must be added there** — `/api/widget`, `/api/stripe/webhook`, `/api/channels` and `/api/mcp` already are.
13. **OpenRouter account setup lives outside the repo**: the account has no purchased credits, so `:free` models are capped at 50 req/day **account-wide** (20/min; UTC reset) — shared by every tenant plus the landing-page live demo, and one chat message costs 2–3 requests because of the tool loop. The key itself carries a $1 credit limit as a second cost guard. Live quota: `free_model_daily_requests` in `GET /api/v1/key`. The primary free model is often unavailable, so the fallback list in `model-router.ts` is load-bearing (first verified request was served by the first fallback).
14. **The public Telegram demo bot (`@ai_support_agent_demo_bot`) is attached to the demo org, and nothing in the UI can manage it.** `requireWriteAccess()` in `integrations/actions.ts` deliberately rejects the demo account, so the channel was created (2026-09-21) by a one-off script that mirrors `saveTelegramChannel()` with the demo org's id. To disconnect: call Telegram `deleteWebhook` with the bot token (in `.env.local` as `TELEGRAM_PROBE_BOT_TOKEN`) **and** delete the demo org's `TelegramChannel` row — doing only one leaves either a dead "connected" row or a webhook pointing at a missing id. Every message it answers spends the shared OpenRouter free quota (gotcha #13), and strangers can create tickets in the demo org through it (the org is on `pro`).
